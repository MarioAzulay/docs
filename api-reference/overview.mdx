...
The IZI Pay API uses HMAC-based authentication. Each application (client) is assigned an `AppId` (client ID) and `AppSecret` (client secret). These credentials must be securely stored and never exposed in public repositories.
...
<CardGroup cols={2}>
  <Card title="References API" icon="link" href="/api-reference/references">
  ...
  <Card title="Callbacks/Webhooks API" icon="bell" href="/api-reference/webhooks">
    Receive real-time event notifications
  </Card>
  <Card title="SMS API" icon="message" href="/api-reference/sms">
...
The API supports webhook event notifications to keep your system updated in real-time. Learn more in the [Webhooks Guide](/webhooks/overview).
